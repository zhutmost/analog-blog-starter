...
authors:
  ...
  - Ron Weasley
  - name: You-Know-Who
    href: https://en.wikipedia.org/wiki/Lord_Voldemort
    bio: The Dark Lord
...
`title` 会作为图片的标题显示在图片下方。如果不设置（就像上面的几个例子），则会显示 `alt` 作为标题。
如果您不需要图片标题（例如一些装饰性图片），可以将 `title` 和 `alt` 两者都不设置。

下面是一个例子：

```markdown
![Image B](392-300x200-grayscale.jpg "Caption of Image B")
```
